...
/**
 * Matches revision texts and math formula and sets the editor association.
 * 
 * @author anna
 *
 */
public class EditorAssociation {
	 ...
	 */
	public void associateTextAndMathEditors(List<List<Token>> orderedTokenizedRevisions, List<List<Token>> orderedMathFormulas,
			TextMatcher matcher, MathMatcher mathMatcher){
	...
	private void updateTokenSources(TextMatcher matcher){
		...
		if(!tokenizedRevisions.isEmpty()){
			...
			List<List<Match>> bestMatches = matcher.match((List) tokenizedRevisions, matcher.getDefaultComparator());
			...
			logger.error("Revision list is empty.");
		}
	}
			...
			for(Object item : revisionFormulas){
				...
				if(formula.getSourceId().getIndex()==revIndex && revIndex!=0){
					...
					List<Match> formulaMatches = mathMatcher.match(
							...
							formulaIndex, 
							revIndex, 
							mathMatcher.getDefaultComparator());
					...
					if(!formulaMatches.isEmpty()){
						...
						formulaMatches.stream().forEach(match -> updateMathSourcesFromMatch((MathMatch) match, formula));
					}
				}
				formulaIndex++;
			}

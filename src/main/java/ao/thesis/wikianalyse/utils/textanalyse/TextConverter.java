...
import ao.thesis.wikianalyse.utils.textanalyse.tokens.Markup;
...
public class TextConverter
		extends
			AstVisitor<WtNode>
{
	private static final Pattern wspunct = Pattern.compile("\\s+|\\p{Z}+"
												+ "|(?=(?U)\\p{Punct})|(?<=(?U)\\p{Punct})"
												+ "|(?=\\p{Punct})|(?<=\\p{Punct})");
...
//	private int positionCounter = 0;
	...
	private List<MathToken> formulaTokens;
...
//		extLinkNum = 1;
	...
	public void visit(WtInternalLink link)
	{
		...
		//TODO not needed for English input data
...
//		}
	...
	public void visit(WtTagExtension n)
	{
		if(("math").equalsIgnoreCase(n.getName())){
			...
			writeMathToken=true;
			formulaTokens = new ArrayList<MathToken>();
			...
			tokens.add(new MathFormula(formulaTokens));
			//wantSpace();
	...
	private void writeWord(String s)
	{
		...
		if(writeMathToken){
			formulaTokens.add(new MathToken(s));
		}
	}
	...
	private void write(String s)
	{
		...
		String[] words = wspunct.split(s.replaceAll("\\s+(?=\\p{Punct})", ""));
		wantSpace();

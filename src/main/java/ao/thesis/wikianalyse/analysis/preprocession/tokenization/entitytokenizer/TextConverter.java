...
//	private final int wrapCol = 80;
	...
	public List<NEToken> netokens = new LinkedList<>();
...
//	private Pattern usedPattern = wspunct;
	...
	TextConverter(CRFClassifier<CoreLabel> classifier){
...
//			finishLine();
	...
	public void visit(WtParagraph p)
	{
		iterate(p);
			...
			int start = 0;
			int end = 0;
			while((start = textWithXML.indexOf("<PERSON>", start)) != -1){
				end = textWithXML.indexOf("</PERSON>", start);
				String namedEntityText = textWithXML.substring(start+8, end);
				
				List<StringToken> elements = new LinkedList<StringToken>();
				String[] words = namedEntityText.split(wspunctPattern);
				for(int i = 0; i < words.length; i++){
					elements.add(new StringToken(words[i]));
				}
				
				NEToken neToken = new NEToken(elements, namedEntityText, "Person");
				netokens.add(neToken);
			...
			while((start = textWithXML.indexOf("<LOCATION>", start)) != -1){
				end = textWithXML.indexOf("</LOCATION>", start);
				String namedEntityText = textWithXML.substring(start+10, end);
			...
			while((start = textWithXML.indexOf("<MISC>", start)) != -1){
				end = textWithXML.indexOf("</MISC>", start);
				String namedEntityText = textWithXML.substring(start+6, end);
